...
// overrides World prototype to alter letAct method
// incorporates concept of energy
...
Carnivore.prototype.act = function(view) {
  ...
  if (this.energy > 90 && space)
   return {type: "reproduce", direction: space};
  let plantEater = view.find("X");
  if (plantEater)
    return {type: "eat", direction: space};
  if (space)
    return {type: "move", direction: space};
};
...
function animateWorld(scenario) {
  for (let i = 0; i < 10; i++) {
    scenario.turn();
...
animateWorld(valley);
console.log(valley.grid.space[0]);
